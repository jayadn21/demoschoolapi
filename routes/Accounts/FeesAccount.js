...
router.post('/getFeesAccounts', function (req, res) {

    try {
        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.execute("FeesAccount_selectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "FeesAccount details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
// 	"Id":"1"
// }
router.post('/getFeesAccount', function (req, res) {
    let Id = req.body.Id;
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("FeesAccount_selectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setFeesAccount', function (req, res) {
    let Id = req.body.Id;
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("FeesAccount_CreateOrUpdate").then(function (results) {
                console.log(results);

                if (results.returnValue != 0) {
                    res.send({
                        success: false,
                        message: "Error while updating FeesAccount.",
                        results: results
                    });
                } else {
                    res.send({
