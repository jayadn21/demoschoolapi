...
router.post('/getFeesHeaders', function (req, res) {

    try {
        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.execute("FeesHeaders_selectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "FeesHeader details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
// 	"Id":"1"
// }
router.post('/getFeesHeader', function (req, res) {
    let Id = req.body.Id;
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("FeesHeaders_selectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setFeesHeader', function (req, res) {
    let Id = req.body.Id;
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.input('InstitutionId', sql.Int, InstitutionId);

            request.execute("FeesHeaders_CreateOrUpdate").then(function (results) {
                console.log(results);

                if (results.returnValue != 0) {
                    res.send({
                        success: false,
                        message: "Error while updating FeesHeaders.",
                        results: results
                    });
                } else {
                    res.send({
