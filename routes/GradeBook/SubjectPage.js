...
router.post('/getSubjects', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Subject_selectAll").then(function (results) {
                console.log(results);
...
router.post('/getSubject', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Subject_selectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setSubject', function (req, res) {
    let Id = req.body.Id;
    let Name = req.body.Name;
    let ShortName = req.body.ShortName;
    let SubjectTypeId = req.body.SubjectTypeId;

    try {
        if (Id == "") throw "Id is empty";
        if (Name == "") throw "Name is empty";
        if (ShortName == "") throw "ShortName is empty";
        if (SubjectTypeId == "") throw "SubjectTypeId is empty";
        ...
        dbConn.connect().then(function () {
            ...
            request.input('SubjectTypeId', parseInt(SubjectTypeId, 10));

            request.execute("Subject_CreateOrUpdate").then(function (results) {
                console.log(results);

                if (results.returnValue != 0) {
                    res.send({
                        success: false,
                        message: "Error while updating Assessment.",
                        results: results
                    });
                } else {
                    res.send({
...
router.post('/delSubject', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Subject_delete").then(function (results) {
                console.log(results);
