...
router.post('/getPatterns', function (req, res) {

    try {
        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.execute("PatternSelectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "Pattern details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
// 	"Id":"1"
// }
router.post('/getPattern', function (req, res) {
    let Id = req.body.Id;
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("PatternSelectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setPattern', function (req, res) {
    let Id = req.body.Id;
    let PatternName = req.body.PatternName;
    try {
        if (Id == "") throw "Id is empty";
        if (PatternName == "") throw "PatternName is empty";

        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.input('Id', parseInt(Id, 10));
            request.input('PatternName', sql.VarChar, PatternName);


            request.execute("PatternCreateOrUpdate").then(function (results) {
                console.log(results);
