...
router.post('/getSubCastes', function (req, res) {

    try {
        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.execute("SubCasteSelectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "SubCaste details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
// 	"Id":"1"
// }
router.post('/getSubCaste', function (req, res) {
    let Id = req.body.Id;
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("SubCasteSelectById").then(function (results) {
                console.log(results);
