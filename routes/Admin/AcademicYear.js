...
// 	"Id":"1"
// }
router.post('/getacademicyear', function (req, res) {
    let Id = req.body.Id;
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("AcademicYear_selectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setacademicyear', function (req, res) {
    let Id = req.body.Id;
    let InstitutionId = req.body.InstitutionId;
    ...
    try {
        ...
        //if (CurrentYear == "") throw "CurrentYear is empty";
        ...
        dbConn.connect().then(function () {
            ...
            request.input('CurrentYear', sql.VarChar, CurrentYear);


            request.execute("AcademicYear_CreateOrUpdate").then(function (results) {
                console.log(results);

                if (results.returnValue != 0) {
                    res.send({
                        success: false,
                        message: "Error while updating Academic Year.",
                        results: results
                    });
                } else {
                    res.send({
