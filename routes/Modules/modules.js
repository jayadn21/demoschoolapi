...
router.post('/getmodules', function (req, res) {
    try {
        var dbConn = new sql.ConnectionPool(localconfig);
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.execute("Module_selectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "Modules details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
router.post('/getmodule', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Module_selectById").then(function (results) {
                console.log(results);
...
*/
router.post('/setmodule', function (req, res) {
    let Id = req.body.Id;
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("ModuleCreateOrUpdate").then(function (results) {
                console.log(results);
