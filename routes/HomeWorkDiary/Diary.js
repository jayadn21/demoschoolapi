...
router.post('/getdiarys', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Diary_selectAll").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "Diarys details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
router.post('/getdiary', function (req, res) {
    ...
    try {
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Diary_selectById").then(function (results) {
                console.log(results);
                ...
                if (results.recordset === [] || results.recordset.length === 0 || results.recordset === undefined
                    ...
                    res.send({
                        success: false,
                        message: "Diary details not found"
                    });
                } else {
                    res.send({
                        success: true,
...
{
    "Id":"-1",
    "DiaryDate":"29-10-2019",
    "DueDate":"10-11-2019",
    "AcademicYearId":"2019-20",
    "ClassSectionMediumId":"1CA",
    "Subject":"Holiday",
...
*/
router.post('/setdiary', function (req, res) {
    let Id = req.body.Id;
    let DiaryDate = req.body.DiaryDate;
    ...
    try {
        if (Id == "") throw "Id is empty";
        if (DiaryDate == "") throw "DiaryDate is empty";
        ...
        dbConn.connect().then(function () {
            var request = new sql.Request(dbConn);
            request.input('Id', sql.Int, parseInt(Id, 10));
            request.input('DiaryDate', sql.VarChar, DiaryDate);
            ...
            request.execute("Diary_CreateOrUpdate").then(function (results) {
                console.log(results);
...
router.post('/deldiary', function (req, res) {
    let Id = req.body.Id;
    try {
        if (Id == "") throw "Id is empty";
        ...
        dbConn.connect().then(function () {
            ...
            request.execute("Diary_delete").then(function (results) {
                console.log(results);
